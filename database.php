<?php
	...
	function saveimage($userid, $imageProperties, $image){
		$db = connectToDB();
		$query = "update usertable set image='{$image}', imageType='{$imageProperties['mime']}' where userid='$userid'";
	...
	function displayimage($userid){
		$db = connectToDB();
		$query = sprintf("select imageType,image from usertable where userid='%s'", $userid);
		$result= mysqli_query($db,$query);
		if ($result) {
			while($row = mysqli_fetch_array($result)) {
				if ($row["imageType"] === null) {
					echo '<img src="default.jpg" style="width:30%; position:relative; left:35%;" align="center">';
				} else {
					echo '<img style="width:30%; position:relative; left:35%;" align="center" src="imageView.php?image_id='.$userid.'" /><br/>';
				}
			}
		}
		mysqli_close($db);
